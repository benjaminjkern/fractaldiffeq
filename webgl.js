...
export default class WebGLShaderRenderer {
    ...
    programInfo = {
        shaders: [{ type: "vertex" }, { type: "fragment" }],
        uniforms: [],
        attributes: ["position"],
    };
    ...
    async setShader(vertex, fragment) {
        this.programInfo.shaders[0].code =
            await WebGLShaderRenderer.downloadShader(vertex);
        this.programInfo.shaders[1].code =
            await WebGLShaderRenderer.downloadShader(fragment);

        this.shaderProgram = this.buildShaderProgram();

        this.gl.enableVertexAttribArray(this.shaderProgram.attributes.position);
        this.gl.vertexAttribPointer(
            this.shaderProgram.attributes.position,
            2,
            this.gl.FLOAT,
            false,
            4 * 4,
    ...
    buildShaderProgram() {
        ...
        let uniforms = Object.fromEntries(
            this.programInfo.uniforms.map((name) => [
                name,
                this.gl.getUniformLocation(program, name),
            ])
        );
        let attributes = Object.fromEntries(
            this.programInfo.attributes.map((name) => [
                name,
